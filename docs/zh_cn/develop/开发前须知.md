...
6. 本地安装
    ...
    3. 将一份嵌入版的python解释器放入`install/python`目录（你可能需要手动创建这个目录）。

        > 建议你使用以下两种方法之一来获取嵌入版的python解释器：
        >
        > 1. 直接把M9A的release包中附带的`python`文件夹复制过来。
        >
        > 2. 手动下载[python-3.10.11-embed](https://mirrors.huaweicloud.com/python/3.10.11/python-3.10.11-embed-amd64.zip)并解压至`install/python/`目录，然后**用它运行[get-pip.py](https://bootstrap.pypa.io/pip/get-pip.py)**。
        ...
        > ```
        >
        > 如果遇到此问题，请**使用`install/python`内的解释器**运行[get-pip.py](https://bootstrap.pypa.io/pip/get-pip.py)。

    然后就可以在 install 文件夹运行 M9A 了。
